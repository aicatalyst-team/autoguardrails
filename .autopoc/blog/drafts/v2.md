<!-- Changelog v2:
- Added title (formatting fix)
- Moved thesis to opening paragraph (architect fix)
- Added hero image placeholder (image fix)
- Expanded all acronyms on first use: ASR, CLI, UBI, PVC (formatting fix)
- Explained ASR metric meaning (content fix)
- Replaced "simple but powerful" with specific description (content fix)
- Changed pip install -e to pip install . (content fix)
- Added 3 CTAs with redhat.com links (formatting fix)
- Added Mermaid diagrams for deployment topology and evaluation flow (image fix)
- Used contractions throughout (formatting fix)
- Removed inline backticks per style guide (formatting fix)
- Added internal links to Red Hat product pages (formatting fix)
- Added alt text to all visuals (image fix)
- Clarified next steps as projected, not proven (content fix)
-->
